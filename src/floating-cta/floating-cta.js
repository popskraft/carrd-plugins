(function() {
  ...
  function isNamedSource(source) {
    return safeNamePattern.test(normalizeName(source.getAttribute('data-floating')));
  }
  ...
  function resolvePositionForViewport(element) {
    ...
    const basePosition = normalizePositionOverride(element.getAttribute('data-floating-position-base'));
  ...
  function syncPositions() {
    trackedElements.forEach(element => {
      element.setAttribute('data-floating-position', resolvePositionForViewport(element));
    });
  }
  ...
  function discoverElements() {
    ...
    sources.forEach(source => {
      if (source.dataset.floatingInitialized === 'true') return;
      ...
      clone.setAttribute('data-floating-position-base', normalizePosition(source.dataset.floatingPosition));
      clone.setAttribute('data-floating-position-mobile', normalizePositionOverride(source.dataset.floatingPositionMobile));
      clone.setAttribute('data-floating-position', normalizePosition(source.dataset.floatingPosition));
      clone.setAttribute('data-floating-hide', normalizeHide(source.dataset.floatingHide));
      clone.setAttribute('aria-hidden', 'true');
      clone.querySelectorAll('[id]').forEach(node => node.removeAttribute('id'));

      source.dataset.floatingInitialized = 'true';
      document.body.appendChild(clone);
    });

    trackedElements = Array.from(document.querySelectorAll('[data-floating-clone="true"]'));
  }
